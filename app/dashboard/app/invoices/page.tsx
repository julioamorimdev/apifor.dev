"use client";
import { badge, card, CardHead, cell, codeDim, Page, PageHead, tableStyle, thCell, usePoll, useT } from "../ui";
    ...
    <Page>
      ...
      <div style={card}>
        ...
        <table style={tableStyle}>
          <thead><tr><th style={thCell}>{t("Data")}</th><th style={thCell}>Valor</th><th style={thCell}>{t("Status")}</th><th style={thCell}>Stripe ID</th><th style={{ ...thCell, textAlign: "right" }}>PDF</th></tr></thead>
          <tbody>
            {list.map((iv, i) => (
            ...
            {!list.length && <tr><td style={cell} colSpan={5}>{t("nenhuma fatura")}</td></tr>}
          </tbody>
        </table>
      </div>
    </Page>
