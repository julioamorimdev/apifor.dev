...
- 🚧 **M5.2** — **rotinas**: trigger **schedule** (scheduler no cérebro dispara a cada N segundos) + **manual** (run via REST); a ação cria uma tarefa e roda o relay. enable/disable/delete; tela **Rotinas**. Validado e2e (schedule redispara, manual cria, disable para). `event` fica p/ depois. Falta **M5.3** (memória/KB), **M5.4** (notificações).
