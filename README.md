...
- 🚧 **M4.1** — **pipeline com gates**: tarefa percorre **plan→exec→test→review→merge** dirigida pelo cérebro, com **gates server-side** (CI verde / revisão IA / **revisão humana**). Intervenção humana destrava o merge (task `blocked` → `/v1/interventions` → cérebro despacha o merge). Telas **Intervenção** + **PRs** (gates `ci`/`ai_review`/`human_review`). Validado e2e: gate humano→merge real no remote, auto-merge (gate off), teste falhando barra o merge. Falta **M4.2**: `agent_profile` (modelo por agente), reconciliação (`TaskStateSnapshot`), telas CI/QA/Logs.
