...
- 🚧 **M5.3** — **memória & KB**: memória da org (escopo **global/repo**) **injetada no prompt de planejamento**; KB importada via **IPC `kb.import`** (arquivo fica local, só o metadado vai ao cérebro) e **consultada pelo agente** no relay; `save_memory` na intervenção salva a decisão. Tela **Conhecimento**. Validado e2e (cérebro injeta a memória; executor confirma memória+KB no prompt). Falta **M5.4** (notificações).
