...
- 🚧 **M6.2** — **security review** ([SECURITY.md](SECURITY.md)) + correções: **`REQUIRE_AUTH`** fecha o fallback dev "demo owner" (401 sem JWT; públicos liberados), **CSV injection** neutralizada no export, **aviso de `JWT_SECRET` fraco** no boot. Enforcement de RLS (policies existem mas o cérebro é superuser; precisa role não-privilegiada + `SET LOCAL`, sem quebrar os workers cross-org), remoção de credenciais demo e cloud/SSO documentados como pendências. Validado e2e.
